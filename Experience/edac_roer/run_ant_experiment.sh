#!/bin/bash
# Ant-v2 完整实验脚本
# 对比EDAC和EDAC+ROER在Ant-v2上的表现

NUM_SEEDS=${1:-5}

echo "================================================"
echo "Ant-v2 对比实验"
echo "种子数量: $NUM_SEEDS"
echo "================================================"

# 激活环境
source ~/miniconda3/etc/profile.d/conda.sh
conda activate roer

cd "$(dirname "$0")"

# EDAC+ROER（使用论文中Ant-v2的参数）
echo ""
echo "运行 EDAC+ROER (Ant-v2)..."
echo "参数: temp=1, max_clip=100, min_clip=10"

for seed in $(seq 42 $((41 + NUM_SEEDS))); do
    echo "  - EDAC+ROER Seed $seed"
    python train_edac_roer.py \
        --env_name=Ant-v2 \
        --seed=$seed \
        --use_roer=True \
        --roer_temp=1.0 \
        --roer_max_clip=100.0 \
        --roer_min_clip=10.0 \
        --roer_per_beta=0.01 \
        --max_steps=1000000 \
        --save_dir=./results/ant_edac_roer/ \
        --tqdm=True &
    
    sleep 5  # 避免同时启动过多进程
done

wait
echo "EDAC+ROER 完成!"

# EDAC baseline
echo ""
echo "运行 EDAC Baseline (Ant-v2)..."

for seed in $(seq 42 $((41 + NUM_SEEDS))); do
    echo "  - EDAC Baseline Seed $seed"
    python train_edac_roer.py \
        --env_name=Ant-v2 \
        --seed=$seed \
        --use_roer=False \
        --max_steps=1000000 \
        --save_dir=./results/ant_edac_baseline/ \
        --tqdm=True &
    
    sleep 5
done

wait
echo "EDAC Baseline 完成!"

# 分析结果
echo ""
echo "================================================"
echo "结果分析"
echo "================================================"

python << 'EOF'
import numpy as np
import glob

def analyze_method(pattern, name):
    files = glob.glob(pattern)
    
    if not files:
        print(f"\n{name}: 未找到结果文件")
        return None
    
    all_curves = []
    final_scores = []
    
    for f in files:
        try:
            data = np.loadtxt(f)
            if len(data) > 0:
                all_curves.append(data)
                final_scores.append(data[-1, 1])
        except:
            pass
    
    if not final_scores:
        print(f"\n{name}: 无有效数据")
        return None
    
    print(f"\n{name}:")
    print(f"  种子数: {len(final_scores)}")
    print(f"  最终性能: {np.mean(final_scores):.1f} ± {np.std(final_scores):.1f}")
    print(f"  范围: [{np.min(final_scores):.1f}, {np.max(final_scores):.1f}]")
    print(f"  中位数: {np.median(final_scores):.1f}")
    
    # 计算方差系数
    cv = np.std(final_scores) / np.mean(final_scores)
    print(f"  变异系数(CV): {cv:.3f}")
    
    return final_scores

baseline_scores = analyze_method(
    "./results/ant_edac_baseline/*/eval_returns.txt",
    "EDAC Baseline"
)

roer_scores = analyze_method(
    "./results/ant_edac_roer/*/eval_returns.txt",
    "EDAC+ROER"
)

# 统计检验
if baseline_scores is not None and roer_scores is not None:
    from scipy import stats
    
    print("\n" + "="*50)
    print("统计检验:")
    
    # t检验
    t_stat, p_value = stats.ttest_ind(roer_scores, baseline_scores)
    print(f"  t-test: t={t_stat:.3f}, p={p_value:.4f}")
    
    if p_value < 0.05:
        if np.mean(roer_scores) > np.mean(baseline_scores):
            print(f"  ✓ EDAC+ROER 显著优于 Baseline (p<0.05)")
        else:
            print(f"  ✗ Baseline 显著优于 EDAC+ROER (p<0.05)")
    else:
        print(f"  - 无显著差异 (p≥0.05)")
    
    # 效果大小
    improvement = (np.mean(roer_scores) - np.mean(baseline_scores)) / np.mean(baseline_scores) * 100
    print(f"  相对提升: {improvement:+.1f}%")

EOF

echo ""
echo "================================================"
echo "实验完成！结果保存在 ./results/"
echo "================================================"

