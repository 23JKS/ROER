...
flags.DEFINE_string('env_name', 'Hopper-v2', 'Environment name.')
...
flags.DEFINE_integer('capacity', int(1e6), 'Replay buffer capacity')
...
_config_path = os.path.join(os.path.dirname(__file__), 'configs', 'sac_default.py')
...
def main(_):
    ...
    save_dir_base = os.path.join(os.path.expanduser('~'), 'roer_output', FLAGS.save_dir.lstrip('./'))
    save_dir = os.path.join(save_dir_base, ts_str)
    
    eval_save_dir = os.path.join(os.path.expanduser('~'), 'roer_output', FLAGS.eval_save_dir.lstrip('./'))
    ...
    if FLAGS.save_video:
        video_train_folder = os.path.join(save_dir_base, 'video', 'train')
        video_eval_folder = os.path.join(save_dir_base, 'video', 'eval')
    else:
    ...
    # Remove parameters that are explicitly passed from kwargs to avoid duplicates
    kwargs_clean = {k: v for k, v in kwargs.items() 
                    if k not in ['actor_lr', 'critic_lr', 'value_lr', 'temp_lr', 
                                'discount', 'tau', 'target_update_period', 
                                'init_temperature', 'loss_temp', 'double_q', 'algo']}
    ...
    for i in tqdm.tqdm(range(1, FLAGS.max_steps + 1),
        ...
        if i % FLAGS.eval_interval == 0:
            ...
            np.savetxt(os.path.join(eval_save_dir, f'{FLAGS.env_name}_{FLAGS.per}_{FLAGS.per_type}_{FLAGS.per_beta}_{FLAGS.gumbel_max_clip}_{FLAGS.temp}_{FLAGS.max_clip}_{FLAGS.min_clip}_{FLAGS.seed}.txt'),
                       eval_returns,
                       fmt=['%d', '%.1f'])
